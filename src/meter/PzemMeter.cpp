...
bool PzemMeter::begin() {
  PM_LOG_INFO(
      "PZEM", "UART_INIT_BEGIN",
      "uart=1 baud=%lu data_bits=8 parity=none stop_bits=1 rx_gpio=%d tx_gpio=%d timeout_ms=%lu retries=%u protocol=pzem-004t-v4",
  ...
  PM_LOG_INFO("PZEM", "UART_READY",
              "uart=1 protocol=modbus-rtu address=248");
  return true;
}
...
MeasurementSnapshot PzemMeter::poll(const std::uint64_t utc_ms,
  ...
  if (error == MeterError::None) {
    ...
    PM_LOG_TRACE(
        "PZEM", "READING",
        "voltage_v=%.1f current_a=%.3f active_power_w=%.1f energy_wh=%llu frequency_hz=%.1f power_factor=%.2f latency_ms=%lu",
    ...
    if (diag::SerialLogger::instance().allow(limiter_key, 10'000U)) {
      PM_LOG_WARN(
          "PZEM", "READ_FAILED",
          "error=%s numeric=%u consecutive=%lu latency_ms=%lu retries=%u hint=%s",
  ...
  if (diag::SerialLogger::instance().allow("pzem_summary", 60'000U)) {
    PM_LOG_INFO(
        "PZEM", "PERIODIC_SUMMARY",
        "requests=%llu successes=%llu timeouts=%llu crc_errors=%llu invalid_frames=%llu consecutive_errors=%lu last_error=%s last_latency_ms=%lu",
...
MeterError PzemMeter::transact(MeasurementSnapshot& result,
  ...
  PM_LOG_TRACE(
      "PZEM", "UART_TX",
      "bytes=8 hex=%02X%02X%02X%02X%02X%02X%02X%02X",
  ...
  if (received > 0) {
    ...
    for (std::size_t index = 0;
         index < received && cursor + 3U < sizeof(frame); ++index) {
      cursor += static_cast<std::size_t>(
          std::snprintf(frame + cursor, sizeof(frame) - cursor, "%02X",
                        response[index]));
    }
