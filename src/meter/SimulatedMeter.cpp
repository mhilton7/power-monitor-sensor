...
MeasurementSnapshot SimulatedMeter::poll(const std::uint64_t utc_ms,
  ...
  sample.power_factor = static_cast<float>(0.96 + 0.02 * std::sin(phase * 0.25));
  sample.active_power_w = sample.voltage_v * sample.current_a * sample.power_factor;
...
MeterMetrics SimulatedMeter::metrics() const { return metrics_; }
