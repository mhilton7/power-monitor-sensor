...
public:
  ...
  bool begin(LogLevel level = static_cast<LogLevel>(PM_SERIAL_LOG_LEVEL));
  ...
  bool writeSetupReady(const char *ssid);
  bool writeSetupPasswordResult(const char *request_id, bool applied);
#if PM_PHYSICAL_ADMIN_RECOVERY
  bool writeAdminPasswordRecoveryReady(const char *request_id,
                                       std::uint32_t window_ms);
  bool writeAdminPasswordRecoveryResult(const char *request_id, bool applied,
...
#define PM_LOG_ERROR_CODE(subsystem, event, numeric, format, ...)              \
  ::pm::diag::SerialLogger::instance().logNumeric(::pm::diag::LogLevel::Error, \
                                                  subsystem, event, numeric,   \
                                                  format, ##__VA_ARGS__)
