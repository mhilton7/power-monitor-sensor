...
bool shouldLog(const LogLevel configured_level,
...
bool sensitiveKey(const char* key) {
  ...
  for (const char* item : sensitive) {
    if (normalized.find(item) != std::string::npos) return true;
  }
  return false;
}
...
void redactSensitiveAssignments(const char* input, char* output,
  ...
  while (input[source] != '\0' && destination + 1U < output_size) {
    const std::size_t token_start = source;
    std::size_t separator = source;
    while (input[separator] != '\0' && input[separator] != '=' &&
           input[separator] != ':' && !tokenDelimiter(input[separator])) {
      ++separator;
    }
    if ((input[separator] == '=' || input[separator] == ':') &&
      ...
      const std::size_t key_length =
          std::min<std::size_t>(separator - token_start, sizeof(key) - 1U);
      std::memcpy(key, input + token_start, key_length);
      if (sensitiveKey(key)) {
        ...
        for (const char character : redacted) {
          if (character == '\0' || destination + 1U >= output_size) break;
          output[destination++] = character;
        }
        bool quoted = input[source] == '"' || input[source] == '\'';
        const char quote = quoted ? input[source++] : '\0';
        while (input[source] != '\0' &&
               ((quoted && input[source] != quote) ||
                (!quoted && !tokenDelimiter(input[source])))) {
          ++source;
        }
        if (quoted && input[source] == quote) ++source;
        continue;
      }
    }
    output[destination++] = input[source++];
...
std::string maskSsid(const std::string& value) {
  ...
  if (value.size() <= 2U) return "**";
  if (value.size() <= 4U) return value.substr(0, 1) + "**" + value.back();
  ...
  switch (reason) {
    ...
    case 2:
      return {"AUTH_EXPIRE", "Authentication expired.",
              "Check signal strength and access point stability.",
              "PM-WIFI-003"};
    case 3:
    ...
    case 7:
      return {"NOT_ASSOCED", "The station is not associated.", "",
              "PM-WIFI-004"};
    case 8:
      return {"ASSOC_LEAVE", "The station intentionally left association.",
              "", "PM-WIFI-099"};
    case 9:
      return {"ASSOC_NOT_AUTHED", "Association was rejected before authentication.",
    ...
    case 19:
      return {"PAIRWISE_CIPHER_INVALID", "The pairwise cipher is unsupported.",
              "Use WPA2-AES or WPA2/WPA3 transition mode.", "PM-WIFI-003"};
    case 20:
      return {"AKMP_INVALID", "The authentication key-management mode is unsupported.",
              "Use WPA2-Personal or WPA2/WPA3 transition mode.",
              "PM-WIFI-003"};
    case 21:
      return {"UNSUPPORTED_RSN_IE_VERSION",
              "The advertised RSN version is unsupported.", "",
              "PM-WIFI-003"};
    case 22:
      return {"INVALID_RSN_IE_CAP", "The advertised RSN capabilities are invalid.",
              "", "PM-WIFI-003"};
    case 23:
    ...
    default:
      return {"UNKNOWN", "The Wi-Fi driver returned an unrecognized reason.",
              "Retain the numeric code when requesting support.",
              "PM-WIFI-099"};
  }
}
...
std::size_t formatLine(char* output, const std::size_t output_size,
                       ...
                       const char* detail) {
  if (output == nullptr || output_size == 0) return 0;
  const int length = std::snprintf(
      ...
      event == nullptr ? "UNSPECIFIED" : event, detail == nullptr ? "" : detail);
  if (length <= 0) {
    output[0] = '\0';
    return 0;
  }
