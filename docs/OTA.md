...
Firmware validates policy/public key before download, uses configured CA/fingerprint, streams to inactive flash with size/time bounds, hashes during write, and finalizes only on exact match. Unsigned, invalid, cross-target/protocol, truncated, oversized, hash-mismatched, and unauthorized downgrade updates fail.
